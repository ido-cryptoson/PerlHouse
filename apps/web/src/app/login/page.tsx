...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (authError) setError("שגיאה בהתחברות עם Google. נסו שוב.");
  ...
  const handleInviteCode = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true); setError(""); setMessage("");
    const { data, error: rpcError } = await supabase.rpc("redeem_invite_code", { code: inviteCode } as any);
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 bg-gradient-to-b from-amber-50 to-stone-50">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full py-3 rounded-xl border border-stone-200 bg-white text-stone-700 font-medium shadow-sm hover:bg-stone-50 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
        >
        ...
        <div className="flex items-center gap-3 my-5">
          ...
          <span className="text-sm text-stone-400">או</span>
          <div className="flex-1 h-px bg-stone-200" />
        </div>
